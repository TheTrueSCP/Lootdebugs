...
public abstract class FuelConsumeItem extends BasicToolItem
{
    ...
    protected void consumeFuel(Player player, Predicate<ItemStack> FuelType, float fuelAmount)
    {
        ...
        if(!fuel.isEmpty())
        {
            float newFuel = ModUtils.ItemNBTHelper.getFloat(fuel, "fuelAmount") - fuelAmount;

            if (newFuel < 0) {
                newFuel = 0;
            }
            ModUtils.ItemNBTHelper.putFloat(fuel, "fuelAmount", newFuel);
        }
    }
    ...
    public static ItemStack getUsableInventoryFuel(Player player, Predicate<ItemStack> FUEL)
    {
        ...
        {
            if(ModUtils.ItemNBTHelper.getFloat(itemstack1, "fuelAmount") > 0)
            {
                return itemstack1;
            }
        }
