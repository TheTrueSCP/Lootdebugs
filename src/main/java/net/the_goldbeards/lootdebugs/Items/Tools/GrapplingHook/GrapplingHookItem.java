...
public class GrapplingHookItem extends BasicToolItem
{
    ...
    public void onUseTick(Level pLevel, LivingEntity pEntity, ItemStack pStack, int pRemainingUseDuration) {
        ...
        if (pStack.hasTag()) {
            ...
            if (pLevel.getEntity((int) ModUtils.ItemNBTHelper.getFloat(pStack,"lootdebugs.grapplinghook.grapplinghookhookentity.id")) != null)
            {
                if (pLevel.getEntity((int) ModUtils.ItemNBTHelper.getFloat(pStack, "lootdebugs.grapplinghook.grapplinghookhookentity.id")) instanceof GrapplingHookHookEntity GH) {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pEntity, InteractionHand pUsedHand)
    {
        ...
        if(pLevel.getEntity((int) ModUtils.ItemNBTHelper.getFloat(pUsedStack,"lootdebugs.grapplinghook.grapplinghookhookentity.id")) != null)
        {
            if (pLevel.getEntity((int) ModUtils.ItemNBTHelper.getFloat(pUsedStack, "lootdebugs.grapplinghook.grapplinghookhookentity.id")) instanceof GrapplingHookHookEntity GH) {
                GH.discard();
            }
        }

        ModUtils.ItemNBTHelper.putFloat(pingItemStack, "lootdebugs.grapplinghook.grapplinghookhookentity.id", grapplingHookHookEntity.getId());//Create fuel with defaut values
