...
public class BeardSlotContainer extends AbstractContainerMenu {
    ...
    public BeardSlotContainer(int windowId, Inventory inv, FriendlyByteBuf extraData) {
    ...
    public BeardSlotContainer(int windowId, Player player) {
        ...
        player.getCapability(BeardSlotCap.BEARD_SLOT).ifPresent(capability -> {
           // slotsInventory = capability.getStacks();
            ...
            while (slot < size) {
                int x = slot % 9;
                int y = slot / 9;
                this.addSlot(new SlotItemHandler(this.slotsInventory, slot, 8 + 18, 17 + 18));
